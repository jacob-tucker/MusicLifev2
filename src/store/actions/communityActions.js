...
export const postMessage = ({ post, community, posts, theName, id }) => {
  return (dispatch, getState) => {
    db.collection('communities').doc(community).set({
      posts: [{ post: post, id: id, author: theName.firstName + ' ' + theName.lastName }, ...posts]
    }, { merge: true }
    ).then(
...
export const deletePost = ({ post, communityName, id, author }) => {
  return (dispatch, getState) => {
    db.collection('communities').doc(communityName).update({
      posts: firebase.firestore.FieldValue.arrayRemove({ author, id, post })
    }).then(
